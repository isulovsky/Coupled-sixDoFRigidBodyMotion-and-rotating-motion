...
#include "coupledsixDoFRigidBodyMotion.H"
...
bool Foam::sixDoFRigidBodyMotion::isPropellerActive() const
{
    return propellerMotion_.propellerMotionActive();
}

Foam::word Foam::sixDoFRigidBodyMotion::getPropellerZoneName() const
{
    return propellerMotion_.propellerZoneName();
}


void Foam::sixDoFRigidBodyMotion::addPropeller(const dictionary& dict)
{
    if (dict.found("propellerMotion"))
     {
        const dictionary& propellerDict = dict.subDict("propellerMotion");

        Info<<"propellerMotion dict is found"<< endl;
        //propellerMotion_ = coupledsixDoFRigidBodyMotion(propellerDict);

        //Info<<"    "<<propellerMotion_<<endl;
     }

}

void Foam::sixDoFRigidBodyMotion::applyPropellerRotation(Foam::pointField& points, Foam::scalar deltaT)
{
    if (isPropellerActive())
    {
        //pointField newPoints = curPoints();
        applyPropellerRotation(points, deltaT);
        
        // Update the mesh with the new points
        // Note: This might need to be adjusted based on how your mesh updates are handled
        //updateMesh(newPoints);
    }
}
...
)
{
    ...
    const vector& shipPosition = centreOfRotation();
    const tensor& shipOrientation = orientation();

    propellerMotion_.updatePosition(shipPosition, shipOrientation);

   if (propellerMotion_.propellerMotionActive())
    {
        // Get the current mesh points
        const pointField& points = mesh().points();
        pointField newPoints = points;

        // Apply rotation to the points in the propeller zone
        labelList cellsInZone = mesh().cellZones()[propellerMotion_.propellerZoneName()];
        forAll(cellsInZone, i)
        {
            const label cellI = cellsInZone[i];
            const labelList& cellPoints = mesh().cellPoints()[cellI];
            forAll(cellPoints, j)
            {
                const label pointI = cellPoints[j];
                propellerMotion_.applyPropellerRotation(newPoints[pointI], deltaT);
            }
        }
...
) const
{
    ...
    //Info <<"initialPoints.size()"<<tab<<initialPoints.size()<<endl;
}
...
/*
   ...
   forAll(points,i)
   {
     if (points[i][0] = points[i][1] = points[i][2]==0)
    {
      Info <<"multiplicaton of coord. is zero"<<endl;
    }
   }
