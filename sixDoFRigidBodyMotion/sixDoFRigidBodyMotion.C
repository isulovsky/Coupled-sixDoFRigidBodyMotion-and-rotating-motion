...
void Foam::sixDoFRigidBodyMotion::createDict()
{
  ...
  if (Pstream::master())
        {
          fileName filePath(time_.globalPath()/"motionData.dat");
          Info << "Writing motion data to " << filePath << endl;
          motionDataFile_.open(filePath.c_str());
          motionDataFile_ << "# Time surge sway heave pitch roll\n";
          motionDataFile_.flush();
        }
...
void Foam::sixDoFRigidBodyMotion::updateDict()
{
    if(time_.foundObject<IOdictionary>("sixDoFMotion"))
    {
     ...
     if (Pstream::master())
       {
            const vector& cor = centreOfRotation();
            const tensor& Q = orientation();
            
            motionDataFile_ 
                << time_.value() << tab
                << cor.x() << tab << cor.y() << tab << cor.z() << tab
                << Q.yz() << tab << Q.xy() << "\n";
